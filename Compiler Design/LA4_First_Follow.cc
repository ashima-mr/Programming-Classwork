...
ParsingTable constructParsingTable(std::map<char, std::set<char>> &first,
    ...
    for (auto &rule : rules) {
        ...
        for (auto &production : rule.second) {
            ...
            if (first[nonTerminal].find('<') != first[nonTerminal].end() && follow[nonTerminal].find('$') != follow[nonTerminal].end()) {
...
void parseLL1(ParsingTable &table, std::string &input) {
    ...
    while (!stack.empty() && inputIndex < input.size()) {
        ...
        if (stackTop == input[inputIndex]) { // If stack top matches input
            ...
            std::cerr << "Error: Mismatch between input '" << input[inputIndex] << "' and stack top '" << stackTop << "'\n";
            return;
        } else if (table[stackTop].find(input[inputIndex]) == table[stackTop].end()) { // If no production rule found
            std::cerr << "Error: No production rule found for input '" << input[inputIndex] << "' and stack top '" << stackTop << "'\n";
            return;
            ...
            if (production != "<") { // If production is not epsilon
                for (int i = production.size() - 1; i >= 0; --i) {
                    stack.push_back(production[i]); // Push production onto stack in reverse order
                }
            }
        }
    }

    if (stack.empty() && inputIndex == input.size()) { // If both stack and input are empty
